...
function normalizeCandidateLimit(value, fallback = 100) {
  const n = Number(value);
...
function recommend(ctx, query, filters = {}, limit = 20, options = {}) {
  ...
  const categoryFilter = filters.category != null ? normalizeText(filters.category) : null;
  const minPrice = filters.minPrice != null ? toFiniteNumber(filters.minPrice) : null;
  const maxPrice = filters.maxPrice != null ? toFiniteNumber(filters.maxPrice) : null;
  ...
  for (const docId of candidateIds) {
    ...
    if (
      sizeFilter &&
      !item.sizes.map((s) => String(s).trim().toUpperCase()).includes(sizeFilter)
    ) {
      continue;
    }
  ...
  const candidateLimit = normalizeCandidateLimit(options.candidateLimit, 100);
